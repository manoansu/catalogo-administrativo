...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(final HttpSecurity http) throws Exception {
        return http
            //CSRF (Cross-Site Request Forgery) is a security mechanism provided by Spring by default.
            // It uses a filter that generates and validates a random CSRF token, which must be sent with sensitive requests (such as POST, PUT, and DELETE)
            // to ensure the request is legitimate.
                .csrf(csrf -> {
                    csrf.disable();
                })
                .authorizeHttpRequests(authorize -> {
                    //Route authorization for each routes.
                    authorize
                ...
                .oauth2ResourceServer(oauth -> {
                    //JWT (JSON Web Token) is a compact, self-contained token used to securely
                    // transmit information and enable stateless authentication between a client and a server.
                    oauth
